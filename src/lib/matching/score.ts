...
export interface MatchScore {
  score: number        // 0-100
  reasons: string[]    // Pourquoi ça match (ex: "Même ville", "Type préféré")
}
...
export function calculateMatchScore(
  ...
  if (prefs.target_city && space.city) {
    ...
    if (spaceCity === prefCity) {
      score += 30
      reasons.push(`Ville : ${space.city}`)
    } else if (spaceCity.includes(prefCity) || prefCity.includes(spaceCity)) {
      score += 15
      reasons.push(`Ville proche : ${space.city}`)
    }
  }
  ...
  if (prefSpaceTypes.length > 0 && space.type) {
    ...
    if (match) {
      score += 25
      reasons.push('Type d\'espace préféré')
    }
  }

  // 3. Budget (20 pts) — compare le prix/jour vs le max_budget préféré
  ...
  if (prefDistricts.length > 0 && space.district) {
    ...
    if (match) {
      score += 15
      reasons.push(`Quartier : ${space.district}`)
    }
  }
...
function parseBudgetValue(budgetStr: string | null): number {
  ...
  // Cherche le dernier nombre dans la string
